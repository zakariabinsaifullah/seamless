...
const NativeIconPicker = ({
    ...
    return (
        <>
            ...
            />
            <BaseControl id="seam-icon-settings" label={label} __nextHasNoMarginBottom>
                <Dropdown
                    popoverProps={{
                    ...
                    className="seam-icon-settings"
                    renderToggle={({ isOpen, onToggle, onClose }) => (
                        <div className="seam-icon-settings__dropdown">
                            <Button
                                onClick={onToggle}
                                aria-expanded={isOpen}
                                className="seam-icon-settings__dropdown-toggle"
                                __next40pxDefaultSize
                                __nextHasNoMarginBottom
                            >
                                {!!(iconName || customSvgCode) ? (
                                    <span className="seam-icon-settings__indicator">
                                        <RenderIcon customSvgCode={customSvgCode} iconName={iconName} size={20} />
                                    </span>
                                ) : (
                                    <span className="seam-icon-settings__indicator disabled"></span>
                                )}

                                <span className="seam-icon-settings__dropdown-label">
                                    {__(iconName || (customSvgCode ? 'Custom SVG' : 'Select Icon', 'seamless'))}
                                </span>
                            </Button>
                            <Button
                                label={__('Browse library', 'seamless')}
                                onClick={() => {
                                    onClose();
                                    openModal('library');
                                }}
                                ...
                                size="small"
                                icon={folderOpen}
                                className="seam-icon-settings__dropdown-more"
                                __next40pxDefaultSize
                                __nextHasNoMarginBottom
                            />
                        </div>
